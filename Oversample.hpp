#ifndef __Oversample_hpp__
#define __Oversample_hpp__
...
Oversample::Oversample(int factor){ //factor currently ignored
  init();
}
void Oversample::init(){
  yup=0;
	ydp = 0;
  xup = 0; 
...
Oversample::~Oversample(){}
  ...
  float Oversample::processSample(float x){
  ...
  float Oversample::oversampledFunction(float x){ // Bypass, replace it with your favourite waveshaper/whatever
    return x;
  }
//~ int main(){
  //~ Oversample os;
  //~ for(int i=0;i<1000;i++){
    //~ float a=os.processSample(1);
    //~ std::cout<<a<<"\n";
  //~ }
  //~ return 0;
//~ }
#endif // __Oversample_hpp__